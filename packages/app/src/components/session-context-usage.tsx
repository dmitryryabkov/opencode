import { Match, Show, Switch, createEffect, createMemo, createResource, type ComponentProps, type JSX } from "solid-js"
...
export function SessionContextUsage(props: SessionContextUsageProps) {
  ...
  const tooltipValue = () => (
    <div class="flex w-[160px] flex-col gap-2">
      ...
      <ContextTooltipRow
        name={language.t("context.usage.currentContextTokens")}
        value={context()?.total.toLocaleString(language.intl()) ?? "0"}
      />
      <Show when={metrics().subagentTokens > 0}>
        <ContextTooltipRow
          name={language.t("context.usage.subagentTokens")}
          value={metrics().subagentTokens.toLocaleString(language.intl())}
        />
        <ContextTooltipRow
          name={language.t("context.usage.totalTokens")}
          value={metrics().totalTokens.toLocaleString(language.intl())}
        />
      </Show>
    </div>
  )
